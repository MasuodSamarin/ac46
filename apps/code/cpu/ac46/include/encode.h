...
#if REC_EN
    #define exit_rec_api(x)         exit_rec(x)
    #define rec_msg_del_api(x,y)    rec_msg_del(x,y)
#else
    #define exit_rec_api(...)
    #define rec_msg_del_api(...)
#endif
/*----------------------------------------------------------------------------*/
/**@brief  录音通道初始化、文件初始化
   @param  RECORD_OP_API *rec_op_api：句柄
   @param  u8 ch_sel：通道选择
   @param  u16 br：mp2的时候它是bitrate。adpcm的时候，它是blockSize,一般配成256/512/1024/2048，超过2048会被限制成2048
   @param  u16 sr：采样率
   @param  ENC_FORMAT format：编码格式 ENC_FORMAT

   @return REC_CTL * ：句柄
   @note   REC_CTL *init_enc(RECORD_OP_API *rec_op_api, u8 ch_sel, u16 br, u16 sr, ENC_FORMAT format);
*/
/*----------------------------------------------------------------------------*/
REC_CTL *init_enc(RECORD_OP_API *rec_op_api, u8 ch_sel, u16 br, u16 sr, ENC_FORMAT format);

void enc_info_init(RECORD_OP_API *rec_op_api, u16 ch_cnt, u16 br, u16 sr);
...
FILE_OPERATE * fop_api_init(void);
